...
import com.niiran.software.solutions.repository.AttendanceRepository;
...
public class AttendanceServiceImpl implements AttendanceService {
    ...
    public Attendance markAttendance(String ohr, LocalDate date, Attendance.Status status,
        ...
        if (!empOpt.isPresent()) {
            log.error("markAttendance:ERROR | EMPLOYEE_NOT_FOUND | ohr:{}",
                    ohr);
            throw new IllegalArgumentException("Employee with ohr " + ohr + " not found.");
        }
        ...
        if (!existingOpt.isPresent()) {
            log.trace("markAttendance:ATTENDANCE_CREATE | ohr:{} | date:{}", ohr, date);
    ...
    public List<Attendance> getAttendance(String ohr, LocalDate start, LocalDate end) {
        ...
        Employee emp = employeeRepository.findByOhr(ohr)
                .orElseThrow(() -> {
                    log.error("GET_ATTENDANCE | Employee not found | ohr: {}", ohr);
    ...
    private Attendance createAbsentRecord(Employee emp, LocalDate date) {
        log.info("CREATE_ABSENT_RECORD | Start | ohr: {} | date: {}", emp.getOhr(), date);
        ...
        log.debug("CREATE_ABSENT_RECORD | Absent record created | ohr: {} | date: {} | status: {}", emp.getOhr(), date, absent.getStatus());
        log.info("CREATE_ABSENT_RECORD | End | ohr: {} | date: {}", emp.getOhr(), date);
        return absent;
    }
    ...
    public List<Attendance> getAttendanceByOhr(String ohr) {
        log.info("GET_ATTENDANCE_BY_OHR | Request received | ohr: {}", ohr);

        List<Attendance> records = attendanceRepository.findByEmployee_Ohr(ohr);

        log.info("GET_ATTENDANCE_BY_OHR | Records fetched | ohr: {} | count: {}", ohr, records.size());

        if (records.isEmpty()) {
            log.warn("GET_ATTENDANCE_BY_OHR | No records found | ohr: {}", ohr);
        } else {
            log.debug("GET_ATTENDANCE_BY_OHR | First record sample | ohr: {} | date: {} | status: {}",
                    ohr,
    ...
    public List<Attendance> getAttendanceBetween(LocalDate fromDate, LocalDate toDate) {
        ...
        if (records.isEmpty()) {
            ...
            log.debug("GET_ATTENDANCE_BETWEEN | Sample record | ohr: {} | date: {} | status: {}",
    ...
    public List<Attendance> getAttendanceForEmployeeBetween(String ohr, LocalDate fromDate, LocalDate toDate) {
        log.info("GET_ATTENDANCE_FOR_EMPLOYEE_BETWEEN | Request received | ohr: {} | fromDate: {} | toDate: {}",
                ohr, fromDate, toDate);
    ...
    public List<Attendance> getAllAttendances() {
        ...
        if (records.isEmpty()) {
            ...
            log.debug("GET_ALL_ATTENDANCES | Sample record | ohr: {} | date: {} | status: {}",
                    records.get(0).getEmployee().getOhr(),
                    records.get(0).getDate(),
                    records.get(0).getStatus());
        }
    ...
    public Workbook writeAttendancesToExcel() {
        ...
        for (Attendance att : attendances) {
            ...
            log.trace("WRITE_ATTENDANCES_TO_EXCEL | Row created | ohr: {} | date: {} | status: {}",
